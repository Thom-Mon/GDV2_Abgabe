...
// -----------------------------------------------------------------------------
// Main Function to provide Fractal
// -----------------------------------------------------------------------------
...
float iterateSierpinski(float2 _coord, int _maxIterations)
{
    float2 z = float2(0, 0);
    for (int i = 0; i < _maxIterations; i++)
    {
...
{
    return float4(iterateSierpinski(_Input.m_PositionNormed, (int) c_PSMaxIteration) * c_PSColor, 1.0f);
}
